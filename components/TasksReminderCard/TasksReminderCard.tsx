    ...
    const onCheck = async (_id: string, isDone:{isDone:boolean}) => {
        try {
          const tasksCollection = await updateTaskById(_id, isDone);
            setTasks((prevTasks) =>prevTasks.map((task) => task._id === _id ? { ...task, isDone: tasksCollection.isDone } : task));
        } catch {
              import("izitoast").then((iziToast) => {
      iziToast.default.error({
        title: "Error",
  ...
  return (
    <>
      <div className={css.titleWrapper}>
        ...
        <button type="button" className={css.addButton}><FiPlusCircle className="addButtonIcon" size={22} onClick={() => setModal(true)}/> </button>
      </div>
                ...
                <li key={_id}>
                    <div className={css.dateWrapper}>{date}</div>
                        ...
                        <div className={`${css.customCheckbox} ${isDone ? css.customCheckboxChecked : ''}`}>
                            <FaCheck className={css.checkIcon} size={12} fill={isDone ? '#FFFFFF' : 'transparent'} /></div>
